...
`ggridges`) were loaded and settings were applied.
...
A three-panel plot that shows the 24-hour activity time course for each
education level with color code corresponding to sex is displayed above.
Becuase this is a plot for 24-hour activity time course, the mims data
every minute was changed to hourly data so that the plot can show the
hourly activity.
